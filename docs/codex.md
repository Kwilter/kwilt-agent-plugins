...
Enable the `Kwilt` MCP server in Codex and complete the hosted OAuth consent flow. Sprint B access reads summarized Kwilt context and, when write scope is granted, lets Codex create, update, or recoverably delete user-owned Arcs, Goals, To-dos, To-do steps, Goal check-ins, focus state, and Chapter user notes for durable planning capture.
